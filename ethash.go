...
func (l *Light) Verify(block Block) bool {
	...
	*/
	...
	if !ok {
		log.Printf("compute !ok")
		return false
	}

	// avoid mixdigest malleability as it's not included in a block's "hashNononce"
	if block.MixDigest() != mixDigest {
		log.Printf("Mix Digest diff %x %x", block.MixDigest(), mixDigest)
		return false
	}

	// The actual check.
	target := new(big.Int).Div(maxUint256, difficulty)
	log.Printf("target %d\nresult %d , diff %d", target, result.Big(), difficulty)
	return result.Big().Cmp(target) <= 0
}

// NICEHASH ShareDiff
// magic numbers from:
// https://github.com/nicehash/nhethpool/blob/master/sharecheck/ethereum.c
